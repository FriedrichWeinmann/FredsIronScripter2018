Describe "Validating the module manifest" {
	...
	$manifest = ((Get-Content "$moduleRoot\FredsIronScripter2018.psd1") -join "`n") | Invoke-Expression
	[version]$moduleVersion = Get-Item "$moduleRoot\FredsIronScripter2018.psm1" | Select-String -Pattern '\$script:PSModuleVersion = "(.*?)"' | ForEach-Object { $_.Matches[0].Groups[1].Value }
